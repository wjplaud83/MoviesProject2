...
public class movieListActivity extends AppCompatActivity {
    ...
    private void attachAdapter(@NonNull final RecyclerView recyclerView, List<movieGeneralModel> movieGeneralModels) {
    ...
    private void drawLayout(@NonNull final RecyclerView recyclerView, movieGeneral mMoviegeneral) {
        List<movieGeneralModel> movieGeneralModels = new ArrayList<movieGeneralModel>();
        Results[] mResult = mMoviegeneral.getResults();
        for (Results result : mResult) {
            movieGeneralModel obj = new movieGeneralModel(result.getTitle(), result.getPoster_path(), result.getVote_average()
                    , result.getId(), result.getVote_count(), result.getRelease_date(), result.getOverview());
            movieGeneralModels.add(obj);
        }
        if (mResult.length > 0) {
            attachAdapter(recyclerView, movieGeneralModels);
        } else {
    ...
    private void FetchMovie(@NonNull final RecyclerView recyclerView, String temp) {
        ...
        if (FLAG_CURRENT != FLAG_FAV) {
            ...
            mMovieAPI.fetchMovies(FLAG_CURRENT, constant.API_KEY, "en", new Callback<movieGeneral>() {
                @Override
